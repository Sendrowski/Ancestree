"""B10 caterpillar benchmark simulation: msprime ARG with ingroup + 3 outgroups.

Same B6-style demography as :mod:`simulate_baseline`: one
ingroup population plus three outgroup populations at standard split
times. The downstream inference subsets the per-site records to
**monoallelic-ingroup** sites only — those are the ones where the
ladder-vs-caterpillar polarisation distinction bites.

Run directly (writes a small default sim)::

    python workflow/scripts/simulate_caterpillar.py

Or via snakemake::

    snakemake -j 1 results/data/caterpillar_sim.trees
"""
import json
...
try:
    ...
    out_trees = "results/data/caterpillar_sim.trees"
    out_meta = "results/data/caterpillar_sim_meta.json"
    n_ingroup = 20
    n_outgroup_pops = 3
    length = 1e6
    mu = 1.25e-8
