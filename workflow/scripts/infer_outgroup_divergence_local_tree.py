"""B8 outgroup-divergence sweep: local-tree inference for one (depth, spacing, n_out) cell.

Local-tree mirror of :mod:`infer_outgroup_divergence_arg`. Loads the
matching ``outgroup_divergence_sim_d{depth}_s{spacing}.trees``, subsets
the outgroups down to ``{n_out}`` populations with
:func:`_outgroup_divergence_common.select_outgroup_pops`, builds
panel-polymorphic :class:`~ancestree.sites.Site` records over
(ingroup + selected outgroups), and runs
:class:`~ancestree.local_tree_inference.LocalTreeInference` on the
genotypes alone: the pairwise-coalescent HMM infers the local trees the
Felsenstein kernel is then evaluated on, so the true ARG is used only
for the truth column. Per-site JSON schema matches
:mod:`infer_outgroup_divergence_arg` so the report reuses the same
...
Run directly (defaults to ``depth=1.5e6, spacing=geometric, n_out=3``)::
    ...
    python workflow/scripts/infer_outgroup_divergence_local_tree.py
"""
import json
import sys
...
from ancestree import JC69, LocalTreeInference, MajorityOutgroupInference, Site
...
try:
    ...
    out_json = (
        f"results/data/outgroup_divergence_local_tree_d{depth_str}_s{spacing}"
        f"_n{n_out}.json"
    )
    mu = 1.25e-8
...
# The simulator's own rate, so the HMM's TMRCA-reset transitions are correctly
# specified: this cell varies outgroup placement, not rate misspecification.
...
# simplify() preserves the requested sample order, so tip i of the simplified
# tree sequence is keep_sample_names[i].
name_for_node = {int(node): keep_sample_names[i]
                 for i, node in enumerate(ts_sub.samples())}

sites: list[Site] = []
for v in ts_sub.variants():
    tip_alleles = {name_for_node[int(node)]: v.alleles[v.genotypes[j]]
                   for j, node in enumerate(ts_sub.samples())}
    if len(set(tip_alleles.values())) < 2:
        continue  # monomorphic across the panel: the HMM emission carries no signal
    sites.append(Site(chrom="1", pos=int(v.site.position),
                      alleles=tuple(v.alleles), tip_alleles=tip_alleles))

print(
    f"Outgroup-divergence local-tree: depth={depth_str}, spacing={spacing}, "
    f"n_out={n_out} ({','.join(selected_outgroup_pops)}), "
    f"{ts_sub.num_samples} haplotypes, {len(sites)} panel-polymorphic sites "
    f"of {ts_sub.num_sites}",
    flush=True,
)

# Window, block width and focal node are the library defaults, matching the ARG
# cell's readout: one tree per 8-SNP window over the density-adaptive ~4-SNP
# emission block, the posterior read at the panel's own root. n_ensemble is the
# manuscript's ensemble size, shared with the heatmap's local-tree row, so every
# figure marginalises each window's call over the same number of genealogies
# drawn from the HMM posterior.
inference = LocalTreeInference(
    sites, JC69(),
    mu=mu,
    rec_rate=rec_rate,
    sample_names=keep_sample_names,
    sequence_length=float(ts_sub.sequence_length),
